...
import hrmenu  from "../Data/hr-menu.json";
import storemenu from "../Data/store-menu.json";
import systemmenu from "../Data/system-menu.json";
import { useState } from "react";
import { useEffect } from "react";
function Sidebar(props) {

    const [isSidebarMini, setIsSidebarMini] = useState(false);
    const [menuData, setMenuData] = useState([...hrmenu?.hrmenu]);
    ...
    function GotoChangeMenu() {
        
        if(localStorage.getItem("role") === '3'){
            setMenuData([...storemenu.storemenu]);
        }
        if(localStorage.getItem("role") === '2'){
            setMenuData([...hrmenu.hrmenu]);
        }
        if(localStorage.getItem("role") === '1'){
            setMenuData([...systemmenu.systemmenu]);
        }
        ...
        <div id="mainSideMenu" className={`sidebar px-4 py-4 py-md-5 me-0 ${isSidebarMini ? "sidebar-mini" : ""}`}>
            <div className="d-flex flex-column h-100">
                <div className="mb-0 brand-icon">
                    ...
                    <span className="logo-text">STARAS</span>
                </div>
